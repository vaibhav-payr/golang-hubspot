...
func (c *Client) SingleEmail(emailRequest SendEmailRequest) error {
...
func (c *Client) CreateOrUpdateContact(emailRequest CreateOrUpdateContactRequest, emailId string) (Response, error) {
	//fmt.Print(emailRequest.Properties)
	req := emailRequest
	...
	response, err := c.doRequest(request{
		URL:          fmt.Sprintf("%s/contacts/v1/contact/createOrUpdate/email/%s/?hapikey=%s", c.baseUrl, emailId, c.apiKey),
		Method:       http.MethodPost,
		Body:         body,
		OkStatusCode: http.StatusOK,
	})
